...
object TemporalTrajectoryFlowTest {
  ...
  def oldcuteTest(dropTableFlag:Boolean): Unit = {
    ...
    val data17: Seq[(Tid, Vector[Itemid])] = Vector(
      ...
      (9, Vector(1)),
      (10, Vector(2)))
    val res17 = CTM.run(droptable = dropTableFlag, minsize = 1, minsup = 1, debugData = data17, bin_s = 1, timeScale = NoScale, returnResult = true)
    require(res17._1 == 7, "Test 17 failed")
    ...
    val data16: Seq[(Tid, Vector[Itemid])] = Vector(
      ...
      (7, Vector(1)),
      (8, Vector(2)))
    val res16 = CTM.run(droptable=dropTableFlag, minsize = 1, minsup = 1, debugData = data16, bin_s = 1, timeScale = NoScale, returnResult = true)
    require(res16._1 == 6, "Test 16 failed")
    ...
    val data15: Seq[(Tid, Vector[Itemid])] = Vector(
      ...
      (5, Vector(0, 2)),
      (6, Vector(2)))
    val res15 = CTM.run(droptable=dropTableFlag,minsize = 1, minsup = 1, debugData = data15, bin_s = 1, timeScale = NoScale, returnResult = true)
    require(res15._1 == 5, "Test 15 failed")
    ...
    val data14: Seq[(Tid, Vector[Itemid])] = Vector(
      ...
      (3, Vector(2, 3, 4, 7)),
      (4, Vector(8, 9)))
    val res14 = CTM.run(droptable=dropTableFlag,minsize = 1, minsup = 3, debugData = data14, bin_s = 1, timeScale = NoScale, returnResult = true)
    require(res14._1 == 1, "Test 14 failed")
    require(res14._2.toSet.equals(Set((RoaringBitmap.bitmapOf(2, 3, 4), 3, 3))), "Test 14 failed")
    ...
    val data12: Seq[(Tid, Vector[Itemid])] = Vector(
      ...
      (2, Vector(1, 2, 3)),
      (3, Vector(1, 2, 3)))
    val res12 = CTM.run(droptable=dropTableFlag, minsize = 1, minsup = 1, debugData = data12, bin_s = 1, timeScale = NoScale, returnResult = true)
    require(res12._1 == 1, "Test 12 failed")
    require(res12._2.toSet.equals(Set((RoaringBitmap.bitmapOf(1, 2, 3), 3, 3))), "Test 12 failed")
    ...
    val data13: Seq[(Tid, Vector[Itemid])] = Vector(
      ...
      (3, Vector(2, 3, 5)),
      (4, Vector(2, 3, 6)))
    val res13 = CTM.run(droptable=dropTableFlag, minsize = 1, minsup = 2, debugData = data13, bin_s = 1, timeScale = NoScale, returnResult = true)
    require(res13._1 == 1, "Test 13 failed")
    require(res13._2.toSet.equals(Set((RoaringBitmap.bitmapOf(2, 3), 2, 4))), "Test 13 failed")
    ...
    val data11: Seq[(Tid, Vector[Itemid])] = Vector(
      ...
      (2, Vector(1, 2, 3)),
      (3, Vector(2, 3)))
    val res11 = CTM.run(droptable=dropTableFlag, minsize = 1, minsup = 1, debugData = data11, bin_s = 1, timeScale = NoScale, returnResult = true)
    require(res11._1 == 2, "Test 11 failed")
    require(res11._2.toSet.equals(Set((RoaringBitmap.bitmapOf(2, 3), 2, 3), (RoaringBitmap.bitmapOf(1, 2, 3), 3, 2))), "Test 11 failed")
    ...
    val data10: Seq[(Tid, Vector[Itemid])] = Vector(
      ...
      (2, Vector(2, 3)),
      (3, Vector(1, 2, 3)))
    val res10 = CTM.run(droptable=dropTableFlag, minsize = 1, minsup = 1, debugData = data10, bin_s = 1, timeScale = NoScale, returnResult = true)
    require(res10._1 == 2, "Test 10 failed")
    require(res10._2.toSet.equals(Set((RoaringBitmap.bitmapOf(2, 3), 2, 3), (RoaringBitmap.bitmapOf(1, 2, 3), 3, 2))), "Test 10 failed")
    ...
    val data8: Seq[(Tid, Vector[Itemid])] = Vector(
      ...
      (4, Vector(2, 5)),
      (5, Vector(1, 2, 3, 5)))
    val res8 = CTM.run(droptable=dropTableFlag, minsize = 1, minsup = 1, debugData = data8, bin_s = 1, timeScale = NoScale, returnResult = true)
    require(res8._1 == 6, "Test 8 failed")
    require(res8._2.toSet.equals(Set(
    ...
    // require(stat8("load") == 21, s"Wrong number of explored nodes all: ${stat8("all")} load: ${stat8("load")} filtered: ${stat8("filtered")}")

//    /* -- Test 07 --------------------------------------------------------------------------------------------------- */
//    val data7: Seq[(CellID, Vector[TrajectoryID])] = Vector(
//      (1, Vector(1200001, 2400002, 3600003)),
//      (2, Vector(1200001, 2400002, 3600003)),
//      (3, Vector(1200003, 2400002, 3600004)))
//    val res7: Array[(RoaringBitmap, Int, Int, Double)] = TemporalTrajectoryFlow.run(droptable=dropTableFlag, sizeMin = 1, supMin = 1, debugData = data7, cellRound = 1, returnResult = true, disabledPruningForComparison = false)
//    require(res7.forall(_._1.toArray.forall(v => v >= 0)), "Test 7 failed")

//    /* -- Test 06 --------------------------------------------------------------------------------------------------- */
//    val data6: Seq[(CellID, Vector[TrajectoryID])] = Vector(
//      (1, "abc".toVector.map(_.toInt)),
//      (2, "bcd".toVector.map(_.toInt)),
//      (3, "ce".toVector.map(_.toInt)),
//      (4, "f".toVector.map(_.toInt)))
//    val res6: Array[(RoaringBitmap, Int, Int, Double)] = TemporalTrajectoryFlow.run(droptable=dropTableFlag, sizeMin = 1, cohMin = 0.5, supMin = 2, debugData = data6, cellRound = 1, returnResult = true, disabledPruningForComparison = false)
//    require(res6._1 == 1, "Test 6 failed")
//    require(res6._2.toSet.equals(Set(
//      (RoaringBitmap.bitmapOf(98, 99), 2, 2, 2.0 / 3)
//      // (RoaringBitmap.bitmapOf(99), 1, 3, 3.0 / 3) // non generata perché b,c è già abbastanza coeso
//    )), "Test 6 failed")

//    /* -- Test 04 --------------------------------------------------------------------------------------------------- */
//    val data4: Seq[(CellID, Vector[TrajectoryID])] = Vector(
//      (1, "abd".toVector.map(_.toInt)),
//      (2, "abcd".toVector.map(_.toInt)),
//      (3, "abcd".toVector.map(_.toInt)),
//      (4, "abcd".toVector.map(_.toInt)),
//      (5, "d".toVector.map(_.toInt)),
//      (6, "de".toVector.map(_.toInt)),
//      (7, "de".toVector.map(_.toInt)),
//      (8, "de".toVector.map(_.toInt)),
//      (9, "de".toVector.map(_.toInt)),
//      (10, "de".toVector.map(_.toInt)),
//      (11, "ab".toVector.map(_.toInt)))
//    val res4 = TemporalTrajectoryFlow.run(droptable=dropTableFlag, sizeMin = 2, cohMin = 0.5, supMin = 4, debugData = data4, cellRound = 1, returnResult = true, disabledPruningForComparison = false)
//    require(res4._1 == 2, "Test 4 failed")
//    require(res4._2.toSet.equals(Set(
//      (RoaringBitmap.bitmapOf(97, 98), 2, 5, 1),
//      (RoaringBitmap.bitmapOf(100, 101), 2, 5, 0.5)
//    )), "Test 4 failed")

    /* -- Test 09 --------------------------------------------------------------------------------------------------- */
    /*res9 = TemporalTrajectoryFlow.run(limit = Int.MaxValue, sizeMin = 1, supMin = 27, cellRound = 1,  tbl = "PAMPA_synthetic_1000000_30_5", returnResult = true)
    require(res9.size == 5, "Test 9.c failed")
    res9 = TemporalTrajectoryFlow.run(limit = Int.MaxValue, sizeMin = 1, supMin = 26, cellRound = 1,  tbl = "PAMPA_synthetic_1000000_30_5", returnResult = true)
    require(res9.size == 37, "Test 9.b failed")
    res9 = TemporalTrajectoryFlow.run(limit = Int.MaxValue, sizeMin = 1, supMin = 15, cellRound = 1, tbl = "PAMPA_breastcancer_positive", returnResult = true)
    require(res9.size == 569, "Test 9.a failed")
    res9 = TemporalTrajectoryFlow.run(limit = Int.MaxValue, sizeMin = 1, supMin = 25, cellRound = 1,  tbl = "PAMPA_synthetic_1000000_30_5", returnResult = true)
    require(res9.size == 163, "Test 9.d failed")*/
    ...
    /*res9 = TemporalTrajectoryFlow.run(limit = Int.MaxValue, sizeMin = 1, supMin = 27, cellRound = 1,  tbl = "PAMPA_synthetic_10000000_30_5", returnResult = true)
    require(res9.size == 50, "Test 9.f failed")
    res9 = TemporalTrajectoryFlow.run(limit = Int.MaxValue, sizeMin = 1, supMin = 25, cellRound = 1,  tbl = "PAMPA_synthetic_10000000_30_5", returnResult = true)
    require(res9.size == 2631, "Test 9.g failed")*/

    /* -- Test 05 --------------------------------------------------------------------------------------------------- */
    // TemporalTrajectoryFlow.run(droptable=dropTableFlag, limit = 100, sizeMin = 10, cohMin = 0.1, supMin = 5, cellRound = 3, tbl = "trajectory.besttrj_standard", returnResult = false, disabledPruningForComparison = false)
    // TemporalTrajectoryFlow.run(droptable=dropTableFlag, limit = 100, sizeMin = 10, cohMin = 0.1, supMin = 4, cellRound = 4, tbl = "trajectory.besttrj_standard", returnResult = true, disabledPruningForComparison = false)
    // TemporalTrajectoryFlow.run(droptable=dropTableFlag, limit = 100, sizeMin = 10, cohMin = 0.1, supMin = 3, cellRound = 5, tbl = "trajectory.besttrj_standard", returnResult = true, disabledPruningForComparison = false)
  }
  ...
  def testAbsoluteContiguityClusters(dropTableFlag:Boolean): Unit = {
    println("---- absolute contiguity test 1 temporal bucket size")
    ...
    dataLoader.loadAndStoreDataset(absoluteContiguityInputSet, absoluteContiguityTableView, startSparkContext())
    ...
    require(absoluteContiguityClusters._1 == expectedContiguityClusters.size,
...
//         | found: $absoluteContiguityClusters""".stripMargin)
  ...
  def testSmootherContiguityClusters(dropTableFlag:Boolean): Unit = {
    ...
    dataLoader.loadAndStoreDataset(inputSet, tableName, startSparkContext())
...
//      s"Clusters does not match with the expected results")
  ...
  def testWeeklyContiguityData(dropTableFlag:Boolean):Unit = {
    ...
    val tableName = s"tmp_$test_name"
    dataLoader.loadAndStoreDataset(inputSet, tableName, startSparkContext(),1)
